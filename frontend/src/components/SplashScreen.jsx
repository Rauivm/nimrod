...
// Frame sequence: base → fire → water → earth → aether → back
const FRAMES = [
  '/splash/f0.jpg',  // base
  '/splash/f1.jpg',  // fire surge
  '/splash/f2.jpg',  // water rise
  '/splash/f3.jpg',  // earth breathe
  '/splash/f4.jpg',  // aether peak
  '/splash/f2.jpg',  // water settle
  '/splash/f0.jpg',  // return
];

// Matching element names shown under the orb during animation
...
// ms each frame holds before crossfading to next
const FRAME_DURATION = 520;
const FADE_DURATION  = 380; // ms CSS transition
...
export default function SplashScreen({ onDone }) {
  ...
  // Frame cycling during 'run' phase
  useEffect(() => {
    if (phase !== 'run') return;

    const totalCycles = FRAMES.length - 1;
    let idx = 0;
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (idx >= FRAMES.length - 1) {
        ...
        // Begin exit after last frame settles
        setTimeout(() => {
          setPhase('exit');
  ...
  return (
    <div className={`splash-root ${phase === 'exit' ? 'splash-exit' : ''}`}>
      ...
      <div className="splash-center">
        ...
        {/* Image orb — two layers crossfading */}
        <div className="splash-orb">
          {/* Current frame */}
          <img
            src={FRAMES[currentFrame]}
            className="splash-img splash-img-current"
          ...
          <img
            ...
            className="splash-img splash-img-next"
            style={{ opacity: fading ? 1 : 0 }}
            alt=""
          />

          {/* Orb overlay — radial vignette */}
          <div className="splash-orb-vignette" />
        </div>

        {/* Element label (floats below orb during cycle) */}
      ...
      <style>{`
        ...
        .splash-bg {
          ...
          background:
            radial-gradient(ellipse at 30% 20%, rgba(255,80,0,0.08) 0%, transparent 55%),
            radial-gradient(ellipse at 70% 80%, rgba(0,120,255,0.10) 0%, transparent 55%),
            radial-gradient(ellipse at 50% 50%, rgba(20,160,20,0.05) 0%, transparent 60%),
            linear-gradient(160deg, #040212 0%, #02010a 50%, #040212 100%);
        ...
        .splash-star {
          ...
          border-radius: 50%;
          background: white;
          animation: twinkle 3s var(--delay) ease-in-out infinite alternate;
          opacity: 0.4;
        }
        @keyframes twinkle {
        ...
        .splash-ring-outer {
          ...
          box-shadow: 0 0 40px rgba(255,140,0,0.12), inset 0 0 40px rgba(255,140,0,0.06);
          animation: ringRotate 12s linear infinite;
        }
        .splash-ring-inner {
          width: 276px; height: 276px;
          border-color: rgba(0,160,255,0.2);
          box-shadow: 0 0 30px rgba(0,160,255,0.10), inset 0 0 30px rgba(0,160,255,0.05);
        ...
        .splash-orb-vignette {
          ...
          border-radius: 50%;
          background: radial-gradient(circle at center, transparent 55%, rgba(2,1,10,0.45) 100%);
          pointer-events: none;
        }
        ...
        .splash-title {
          ...
          text-shadow: none;
          filter: drop-shadow(0 0 30px rgba(201,140,30,0.6))
                  drop-shadow(0 2px 6px rgba(0,0,0,0.9));
          margin: 0; line-height: 1;
        }
        .splash-subtitle-line {
          width: 120px; height: 1px; margin: 14px auto 12px;
          background: linear-gradient(90deg, transparent, rgba(201,168,76,0.5), transparent);
        }
        .splash-subtitle {
          font-family: 'Crimson Pro', serif;
          font-size: 13px; letter-spacing: 2px;
          color: rgba(200,180,130,0);
          font-style: italic;
