 ...
 * LgpdConsentModal
 *
 * Informational only — no API requests, no blocking, no reloads.
 * Shown once per browser via localStorage('lgpd_seen').
 * Closing/accepting calls props.onClose which sets the flag upstream.
 */
export default function LgpdConsentModal({ onClose }) {
  return (
    <div className="lgpd-overlay">
      <div className="lgpd-modal animate-in">
        ...
        <p className="lgpd-body">
          Ao usar o <strong>Nimrod</strong>, você concorda que armazenamos
          seus dados de identificação (e-mail e nome) para fins de autenticação,
          conforme a&nbsp;
      ...
      <style>{`
        ...
        .lgpd-icon { color: var(--gold); opacity: 0.8; }
        .lgpd-title {
        ...
        .lgpd-accept-btn:active { transform: translateY(0); }
      `}</style>
    </div>
  );
}
